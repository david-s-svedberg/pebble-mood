...
{
    if(m_metrics_group->alarm.active)
    {
        reschedule_alarm(&m_metrics_group->alarm);
    } else
    {
        unschedule_alarm(&m_metrics_group->alarm);
    }
    metrics_groups_save();
...
static void decrease_time(ClickRecognizerRef recognizer, void* context)
{
    if(m_current_edit_step == 0)
    {
        if(m_metrics_group->alarm.time.hour > 0)
        {
            m_metrics_group->alarm.time.hour--;
        } else
        {
            m_metrics_group->alarm.time.hour = 23;
        }
    } else if(m_current_edit_step == 1)
    {
        if(m_metrics_group->alarm.time.minute > 0)
        {
            m_metrics_group->alarm.time.minute--;
        } else
        {
            m_metrics_group->alarm.time.minute = 59;
        }
    }
    update_edit_alarm_time_layers();
}
...
static void goto_next_edit(ClickRecognizerRef recognizer, void* context)
{
    ...
    if(m_current_edit_step > 1)
    {
        ...
        m_metrics_group->alarm.active = true;
        update_alarm_active_text();
